...
export default function Portfolio() {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-base-content mb-2">Investor Portfolio</h1>
        <p className="text-base-content/70">Manage your holdings, track performance, and view on-chain ledger history.</p>
      </div>
            ...
            <div key={idx} className="card bg-white shadow-sm border border-base-300 flex flex-row overflow-hidden h-48 hover:shadow-md transition-shadow">
              ...
              <div className="card-body p-5 w-3/5">
                <div className="flex justify-between items-start mb-1">
                  <h3 className="card-title text-lg text-base-content leading-tight">{prop.name}</h3>
                </div>
                ...
                <div className="grid grid-cols-2 gap-y-2 text-sm mt-4">
                  <div>
                    <p className="text-base-content/50 text-xs font-medium">Current Value</p>
                    <p className="font-bold text-base-content">{prop.currentValue}</p>
                  </div>
                  <div>
                    <p className="text-base-content/50 text-xs font-medium">Est. Yield</p>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-base-300 overflow-hidden">
        <div className="p-6 border-b border-base-200 flex justify-between items-center">
          ...
          <button className="btn btn-sm btn-outline border-base-300 text-base-content/70 hover:bg-base-200 hover:border-base-300">
            Export CSV
          </button>
        </div>
            ...
            <tbody>
              {ledgerActivity.map((tx, idx) => (
                <tr key={idx} className="hover:bg-base-200/30 transition-colors border-b border-base-200 last:border-0">
                  <td>
                    <div className="font-semibold text-base-content">{tx.event}</div>
                  ...
                  <td className="text-base-content/80 flex items-center gap-2 mt-2">
                    <CubeIcon className="w-4 h-4 text-base-content/50" />
                    {tx.asset}
                  </td>
                  <td className={`font-medium ${tx.amount.startsWith('+') ? 'text-success' : 'text-base-content'}`}>
                    {tx.amount}
                  </td>
